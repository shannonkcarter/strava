...
strava_hc <- hc_theme_merge(
  hc_theme_smpl(),
  hc_theme(colors = c("#ffffff", "#D3D3D3"),
...
strava <- df %>% 
  ...
  mutate(distance_mi = round(distance/1609.34, 1),
...
strava %>% 
  ...
  geom_boxplot(outlier.color = "#ffffff", color = "white", fill = "gray") +
  strava_gg() +
...
strava %>% 
  ...
  hchart("scatter", hcaes(x = suffer_score, y = average_heartrate, color = type, size = distance)) %>% 
  hc_tooltip(formatter = JS("function(){
                                return (this.point.name + 
                            ' <br> Sport: ' + this.point.type +
  ...
  hc_add_theme(strava_hc) %>% 
  #hc_colors(colors = c("#ffffff", "#D3D3D3")) %>% 
  hc_title(text = "What's a suffer score?")
```
```{r}
strava %>% 
  filter(type %in% c("Ride", "Run")) %>% 
  hchart("scatter", hcaes(x = suffer_score, y = max_heartrate, color = type, size =)) %>% 
  hc_tooltip(formatter = JS("function(){
                                return (this.point.name + 
                            ' <br> Sport: ' + this.point.type +
...
strava %>% 
  filter(type == "Run") %>% 
  hchart("scatter", hcaes(x = distance_mi, y = average_heartrate)) %>% 
  hc_tooltip(formatter = JS("function(){
                                return (this.point.name + 
                            ' <br> Sport: ' + this.point.type +
                                      ' <br> Max Heart Rate: ' + this.y + 
...
strava %>% 
  filter(type %in% c("Ride", "Run")) %>% 
  hchart("scatter", hcaes(x = max_heartrate, y = average_heartrate, color = type, size = distance)) %>% 
  hc_tooltip(formatter = JS("function(){
                                return (this.point.name + 
                            ' <br> Sport: ' + this.point.type +
                                      ' <br> Max Heart Rate: ' + this.y + 
...
calendarHeat(tiles$date, tiles$distance_mi, ncolors = 99, color = "r2g", varname="Running & Cycling Miles")
```
...
calendarHeat(tiles$date, tiles$distance_mi, ncolors = 99, color = "r2g", varname="Cycling Miles")
```
```{r}
...
ggplot(pace, aes(x = date, y = average_heartrate, size = distance)) + 
  geom_point(color = "#ffffff", alpha = 0.5) + 
  strava_gg() +
