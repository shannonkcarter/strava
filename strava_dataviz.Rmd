...
cumulative_miles <- strava %>% 
  ...
  hc_add_theme(strava_hc) %>% 
  hc_title(text = "")  %>% 
  hc_colors(c("yellow", "#358856", "dodgerblue", "purple"))
cumulative_miles
```
...
ridge_ss <- strava %>% 
  ...
  mutate(month = as.factor(month(date, label = T, abbr = T)), 
         year = as.factor(year(date)),
         max_hr_over_distance = max_heartrate / distance) #%>% 
  group_by(year, month) %>% 
  mutate(suffer_score = mean(suffer_score, na.rm = T)) %>% 
  select(month, year, suffer_score) %>% 
  distinct()

ggplot(ridge_ss, aes(x = suffer_score, y = month, fill = stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_gradient(low = "yellow", high = "red") + 
  #scale_fill_viridis_c(option = "C") +
  labs(x = "MILES", y = "") +
...
box <- ridge %>% 
  ...
  labs(x = "", y = "MILES", fill = "") +
  scale_fill_manual(values = c("tomato", "yellow", "aquamarine4", "dodgerblue" ,  "purple")) +
