...
running = strava %>% 
  ...
  select(name,
         ...
         moving_time, distance, min_per_mile, total_elevation_gain, 
...
lm_fit <- linear_reg() %>% 
  set_engine("lm") %>% 
  fit(kudos_count ~ distance, data = running)
lm_fit
tidy(lm_fit)
...
pinwheel = ggplot(weeks_run, aes(x = week_number, y = weekly_miles, fill = forcats::fct_rev(as.factor(year)))) + 
  ...
  scale_fill_manual(values = c("dodgerblue", "aquamarine4", "yellow", "tomato")) +
